...
def draw_two_boxes(img_path, bbox_det, prompt, exp_id, output_dir, gt_box=None):
    ...
    if bbox_det:
        draw.rectangle(bbox_det, outline="red", width=3)
    ...
    print(f":white_tick: Saved: {out_path}")
    return out_path
...
print(f"\n:magnifying_glass: Evaluating: {json_name}")
...
with open(out_csv, 'w', newline='') as csv_file, open(out_fail, 'w', newline='') as csv_fail_file:
    ...
    for exp_id, data in tqdm(json_data.items()):
        ...
        # coords = run_image_prompt(args)
        coords = outputs
        bbox = np.array(ast.literal_eval(coords))

        gt_mask = get_gt_mask(img_mask_path)
        gt_box = get_bbox_from_mask(gt_mask)

        bboxes = phrase_grounding_dino(img_color_path, prompt)
        iou_scores = [0.0 for _ in thresholds]
        iou_matched_flags = [False for _ in thresholds]
        matched_objs = []
        matched_objects = []
        ...
        result_entry = {
            ...
            "px_object_id": "|".join(map(str, matched_objs)) if matched_objs else "None",
            "utterance": utterance,
