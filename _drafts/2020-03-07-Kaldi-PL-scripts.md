[[https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech\_tools/tools/ForcedAlign/run.sh]{.underline}](https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech_tools/tools/ForcedAlign/run.sh)
...
\${retry\_beam} data lang tri3b\_mmi ali

./steps/get\_train\_ctm.sh data lang ali

./local\_utils/get\_phoneme\_ctm.sh data lang ali

awk \'\$0=\"@\"\$0\' ali/phonectm \| cat ali/ctm - \| sort -r -k3n \>
\$out
...
[[https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech\_tools/tools/ForcedAlign/run\_eaf.sh]{.underline}](https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech_tools/tools/ForcedAlign/run_eaf.sh)
...
echo \"Usage: ./run\_eaf.sh \<input-wav\> \<input-eaf\> \<out-eaf\>\"
...
eaf\_in=\$(readlink -f \$2)

eaf\_out=\$(readlink -f \$3)
...
eaf\_in=\$data\_path/\$2

eaf\_out=\$data\_path/\$3

fi

for f in \$wav\_file \$eaf\_in; do
...
echo input \$wav\_file \> data/wav.scp

echo \"input input A\" \> data/reco2file\_and\_channel

if \$spk\_tier ; then

python3 local\_utils/eaf2data.py \--spk-tier \--skip-tiers
\"\$skip\_tiers\" \$eaf\_in data

else

python3 local\_utils/eaf2data.py \--skip-tiers \"\$skip\_tiers\"
\$eaf\_in data
...
./utils/utt2spk\_to\_spk2utt.pl data/utt2spk \> data/spk2utt

./steps/make\_mfcc.sh \--nj \$nj data
...
./steps/align\_fmllr.sh \--nj \$nj \--beam \${beam} \--retry-beam
\${retry\_beam} data lang tri3b\_mmi ali

./steps/get\_train\_ctm.sh \--use-segments false data lang ali

if \$phones ; then

./local\_utils/get\_phoneme\_ctm.sh \--use-segments false data lang ali

python3 local\_utils/ctm2eaf.py \--phones-ctm ali/phonectm ali/ctm
data/seg2tier \${eaf\_in} \${eaf\_out}

else

python3 local\_utils/ctm2eaf.py ali/ctm data/seg2tier \${eaf\_in}
\${eaf\_out}
...
[[https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech\_tools/tools/ForcedAlign/run\_segments.sh]{.underline}](https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech_tools/tools/ForcedAlign/run_segments.sh)
...
echo \"Usage: ./run.sh \<input-wav\> \<seg-txt\> \<spk\_ctm\>
\<out-ctm\>\"
...
spk\_ctm=\$data\_path/\$3

out=\$data\_path/\$4

fi

for f in \$wav\_file \$txt\_file \$spk\_ctm; do
...
echo \"input input A\" \> \${tmp\_path}/data/reco2file\_and\_channel

awk \'{printf \"seg\_%s\_%03d input %0.2f %0.2f\\n\", \$5, NR, \$3,
\$3+\$4}\' \< \$spk\_ctm \> \${tmp\_path}/data/segments

awk \'{printf \"seg\_%s\_%03d spk\_%s\\n\", \$5, NR, \$5}\' \<
\$spk\_ctm \> \${tmp\_path}/data/utt2spk

./utils/utt2spk\_to\_spk2utt.pl \< \${tmp\_path}/data/utt2spk \>
\${tmp\_path}/data/spk2utt

cut -f1 -d\' \' \${tmp\_path}/data/utt2spk \> \${tmp\_path}/data/utt.id

paste -d \' \' \${tmp\_path}/data/utt.id \${txt\_file} \>
\${tmp\_path}/data/text

for f in \${tmp\_path}/data/\* ; do
...
[[https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech\_tools/tools/SegmentAlign/run.sh]{.underline}](https://github.com/speech-clarin-pl/SpeechToolsWorkers/blob/master/speech_tools/tools/SegmentAlign/run.sh)
